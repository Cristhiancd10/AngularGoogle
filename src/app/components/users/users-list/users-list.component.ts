...
export class UsersListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  //users: User[] = [];
  constructor(private userService: UsersService,  private activatedRoute: ActivatedRoute, private router: Router,private dialog: MatDialog){
    const storedRole = localStorage.getItem('role');
    this.role = storedRole !== null ? storedRole : '';
    console.log("role Local ", this.role);
    if (this.role==="Administrador") {
      this.displayedColumns= ['idUser', 'id_Card', 'username', 'password',
      'role', 'names', 'phone', 'email', 'city', 'registration_date', 'acciones'];
    }else{
      this.displayedColumns= ['idUser', 'id_Card', 'username', 'password',
      'role', 'names', 'phone', 'email', 'city', 'registration_date'];
    }
  }
...
delete(id: number) {
  ...
  getUserList() {
    this.subRef$=this.userService.GetAllUser()
    .subscribe({
      next: (user) => {
